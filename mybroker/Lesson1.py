...
# 按股票代码，依次循环传入数据
count = 0
total = len(daily_price['sec_code'].unique())
for stock in daily_price['sec_code'].unique():
    ...
    count += 1
    print(f"{count}/{total}: {stock} Done !")
...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        print("调仓日：", self.trade_dates)

        self.order_list = []  # 记录以往订单，方便调仓日对未完成订单做处理
        self.buy_stocks_pre = []  # 记录上一期持仓

    def next(self):
        curdate = self.datas[0].datetime.date(0)  # 获取当前的回测时间点
        pddt = pd.to_datetime(curdate)
  
        # 如果是调仓日，则进行调仓操作
        if pddt in self.trade_dates:
            print("--------------{} 为调仓日----------".format(pddt))
            ...
            # 打印调仓日的调仓列表
            print(self.buy_stock)

            # 提取当前调仓日的持仓列表
            buy_stocks_data = self.buy_stock.query(f"trade_date==@curdate")
